...
(() => {
  ...
  window.dealHoleCards = function(scenario, onComplete) {
    ...
    const activeSeatIds = [0];
    (scenario.players || []).forEach(p => {
      if (p.hand && p.hand.length > 0) {
        const displaySeat = getDisplaySeatId(p.seat);
        if (displaySeat !== 0) activeSeatIds.push(displaySeat);
      }
    });
  ...
  function renderOpponentStyles(players){
    ...
    players.forEach(p => {
      const displaySeat = getDisplaySeatId(p.seat);
      if(displaySeat === 0) return; // hero
      const styleEl = document.getElementById(`style-p${displaySeat}`);
      const actEl   = document.getElementById(`act-p${displaySeat}`);
      if(!styleEl) return;
      if(!p.hand || p.hand.length===0){
        styleEl.textContent="OUT"; styleEl.className="style-badge fold";
      } else {
        styleEl.textContent=p.style||""; styleEl.className="style-badge";
      }
      if(actEl){ actEl.textContent=""; actEl.className="action-badge"; }
    });
  }
  ...
  function markFoldedPlayers(players){
    ...
    players.forEach(p=>{
      if(!p.hand||p.hand.length===0){
        ...
        const el=document.getElementById(`p${displaySeat}`);
  ...
  function resetTable(){
    ...
    // Pot display reset handled by initPotAndStacks
    ...
    for(let i=2;i<=8;i++){
      ...
      const sb = document.getElementById(`style-p${i}`);
      if(sb){ sb.textContent=""; sb.className="style-badge"; }
  ...
  let stacks = {};
  let heroSeat = 1;
  const TABLE_ORDER = [2, 3, 4, 5, 6, 7, 8, 1];

  /* Position rotation: table rotates so hero is always at bottom. Display seats 2-8 show other players clockwise (next to act = left of hero). */
  function getDisplaySeatId(playerNum) {
    if (playerNum === heroSeat) return 0;
    const idx = TABLE_ORDER.indexOf(heroSeat);
    const k = (TABLE_ORDER.indexOf(playerNum) - idx + 8) % 8;
    return k + 1;
  }
  window.getDisplaySeatId = getDisplaySeatId;
  function getPlayerAtDisplaySeat(displaySeat) {
    if (displaySeat === 0) return heroSeat;
    const idx = TABLE_ORDER.indexOf(heroSeat);
    return TABLE_ORDER[(idx + displaySeat - 1) % 8];
  }
  function getStyleBadgeEl(displaySeat) {
    return displaySeat === 0 ? null : document.getElementById(`style-p${displaySeat}`);
  }
  function getActionBadgeEl(playerNum) {
    const seat = getDisplaySeatId(playerNum);
  ...
  window.initPotAndStacks = function(scenarioPot, existingStacks, heroSeatForMapping) {
    ...
    if (existingStacks && Object.keys(existingStacks).length > 0 && heroSeatForMapping != null) {
      heroSeat = heroSeatForMapping;
      stacks = {};
      for (let n = 1; n <= 8; n++) {
        const displaySeat = getDisplaySeatId(n);
        stacks[displaySeat] = existingStacks[n] != null ? existingStacks[n] : 100;
      }
    } else if (existingStacks && Object.keys(existingStacks).length > 0) {
  ...
  // Returns current stacks
  window.getStacks = function() { return { ...stacks }; };
  window.getPotBB = function() { return potBB; };
  ...
  function renderAllStacks() {
    Object.entries(stacks).forEach(([seat, bb]) => {
      ...
      if (!el) return;
      const n = Number(bb);
      el.textContent = n <= 0 ? "ALL IN" : n.toFixed(1) + " BB";
    });
  }
  ...
  function renderStack(seatId) {
    ...
    const el = document.getElementById(id);
    if (!el) return;
    const bb = stacks[seatId] || 0;
    el.textContent = bb <= 0 ? "ALL IN" : Number(bb).toFixed(1) + " BB";
  }
  ...
  function applyBet(playerNum, amountPct) {
    const seatId  = getDisplaySeatId(playerNum);
    const currentStack = Math.max(0, stacks[seatId] ?? 100);
    if (currentStack <= 0) return; /* already all-in */
    const basePot = potBB > 2 ? potBB : 10;
    const rawBet  = Math.max(2, basePot * (amountPct / 100));
    const betBB   = Math.min(rawBet, currentStack);
    lastBetBB = betBB;
    stacks[seatId] = currentStack - betBB;
    if (stacks[seatId] < 0) stacks[seatId] = 0;
    potBB += betBB;
  ...
  function applyCall(playerNum) {
    ...
    const callBB = Math.min(lastBetBB || 2, currentStack);
    stacks[seatId] = currentStack - callBB;
    if (stacks[seatId] < 0) stacks[seatId] = 0;
    potBB += callBB;
    flashStack(seatId, "losing");
    renderPot();
  ...
  function showSeatAction(act){
    ...
    const el  = getActionBadgeEl(num);
    if(!el) return;

    const seatId = getDisplaySeatId(num);
    const currentStack = Math.max(0, stacks[seatId] ?? 100);
    ...
    /* Already all-in: show ALL IN and skip applying bet again */
    if ((act.type === "bet" || act.type === "raise") && currentStack <= 0) {
      label = "ALL IN";
      cls = "raise";
    }
  ...
  function animateFold(num){
    const displaySeat = getDisplaySeatId(num);
    const seat = (displaySeat === 0)
  ...
  */
  window.renderScenario = function(scenario, existingStacks){
    heroSeat = scenario.heroSeat != null ? scenario.heroSeat : 1;
    resetTable();
    ...
    const initialPot = scenario.initialPot != null ? scenario.initialPot : (scenario.pot ?? 0);
    const initialStacks = scenario.initialStacks || (scenario.stacks ? { ...scenario.stacks } : null);
    if (initialStacks && scenario.heroSeat != null)
      window.initPotAndStacks(initialPot, initialStacks, scenario.heroSeat);
    else
    ...
    const POS_BY_SEAT = { 1:"BTN", 2:"SB", 3:"BB", 4:"UTG", 5:"UTG+1", 6:"MP", 7:"HJ", 8:"CO" };
    const pos0 = document.getElementById("pos-0");
    if(pos0) pos0.textContent = (scenario.positions && scenario.positions[heroSeat]) || POS_BY_SEAT[heroSeat] || "—";
    const heroNameEl = document.getElementById("name-hero");
    if (heroNameEl) {
      const heroPlayer = (scenario.players || []).find(p => p.seat === heroSeat);
      heroNameEl.textContent = (heroPlayer && heroPlayer.style) ? heroPlayer.style : "—";
    }
    for (let s = 2; s <= 8; s++) {
      const nameEl = document.getElementById("name-p" + s);
      if (nameEl) nameEl.textContent = "";
      const playerNum = getPlayerAtDisplaySeat(s);
      const posEl = document.getElementById("pos-" + s);
      if (posEl) posEl.textContent = (scenario.positions && scenario.positions[playerNum]) || POS_BY_SEAT[playerNum] || "";
    }
  ...
  */
  ...
  /*
    loadScenarioAtStreet(scenario, onHeroTurn)
    ───────────────────────────────────────────
    Load a scenario that starts at a random street (flop/turn/river).
    Renders table, shows board up to current street, plays all action history
    for previous streets, then on current street pauses at hero's turn and
    calls onHeroTurn(resumeFn). Used by play.html single-decision mode.
  */
  window.loadScenarioAtStreet = function(scenario, onHeroTurn){
    if(!scenario) return;
    ...
    if(scenario.hero?.[1]) renderCard(hc2, scenario.hero[1]);
    renderCard(document.getElementById("mini-card1"), scenario.hero?.[0]);
    renderCard(document.getElementById("mini-card2"), scenario.hero?.[1]);
  ...
  */
  window.showHeroActionBadge = function(actionType){
    ...
    if(chip) {
      ...
      if(actionType==="bet33")  applyBet(heroSeat, 33);
      else if(actionType==="bet50")  applyBet(heroSeat, 50);
      else if(actionType==="bet75")  applyBet(heroSeat, 75);
      else if(actionType==="bet100") applyBet(heroSeat, 100);
      else if(actionType==="allin")  applyBet(heroSeat, 200);
      else if(actionType==="call")   applyCall(heroSeat);
      else if(actionType==="raise")  applyBet(heroSeat, 50);
    } else {
